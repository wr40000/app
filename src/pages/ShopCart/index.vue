<template>
  <div class="cart">
    ...
    <div class="cart-tool">
      ...
      <div class="money-box">
        <div class="chosed">已选择 <span>{{ hasCheck }}</span>件商品</div>
        <div class="sumprice">
          <em>总价（不含运费） ：</em>
...
export default {
  ...
  methods: {
    ...
    async updateChecked(cart, $event){
      ...
      let check = $event.target.check == true ? "1" : "0";
      try{
  ...
  computed: {
    ...
    hasCheck() {
      return this.cartInfoList.reduce((accumulator, currentValue)=>{
         return currentValue.isChecked + accumulator
      }, 0);
    },
    isAllCheck() {
      //判断底部复选框是否勾选【全部产品都选中，采勾选】且购物车不为空才勾选
